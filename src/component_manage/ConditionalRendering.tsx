...
function AndComponent ({number} : {number : number}) {
    // && 연산자를 이용한 조건부 렌더링
    // - JSX 내부(retunr 내부)에서 상황(조건)에 따라 렌더링 여부를 결정하고 싶을 때 사용

    // ex) 만약 number가 양수면 '양수'를 렌더링
    // if ( number > 0) return '양수';
    // 양수가 아니면 '양수가 아님'을 렌더링
    // if (nubmer <= 0) return '양수가 아님';
    // if(!(number > 0 )) retunr '양수가 아님';
    return (
        <h1>
            {number > 0 && '양수'}
            {number <= 0 && '양수가 아님'}
...
function ThreeTermComponent ({number} : {number : number}) {
    // 삼항연산자(? :)를 이용한 조건부 렌더링
    // - JSX 내부(return 내부)에서 상황(조건)에 따라 다른 서로 다른 렌더링을 하고 싶을 때 사용

    // ex) 만약 number가 양수면 '양수', 음수면 '음수', 0이면 '영' 을 렌더링
    return(
        <h1>
