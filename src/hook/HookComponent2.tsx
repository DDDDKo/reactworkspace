...
export default function HookComponent2() {
    ...
    const onButtonClickHandler = () => {
        ...
        // 참조변수(null 혹은 undefined를 가질 수 있는 object타입의 변수)에서
        // 특정 속성 및 메서드를 호출하려할때 null 혹은 undefined가 아닌 상태에서만
        // 작업을 수행하도록 하려면 참조변수?.속성 or 참조변수?.함수()로 실행 => 이형태는 안정적이지 못함
        // inputRef.current?.focus();
        if (inputRef.current) inputRef.current.focus();
    };

    // 첨부한 파일 미리보기 화면 설정
    const onInputChageHandler = () => {
    ...
    return (
        <div>
            <img src={image} />
            <input ref={inputRef} type="file" onChange={onInputChageHandler}/>
