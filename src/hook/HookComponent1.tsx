...
// - useEffect : 컴포넌트가 렌더링될 때마다 특정 작업을 수행할 수 있도록 하는 함수 => mount, update, unmount 단계가 있음 각 단계에서 수행 되는 함수를 나타냄
// - useRef : DOM 요소에 직접 접근하기 위한 함수, => 렌더링 사이에 값을 기억하는 함수
// - useContext : 컴포넌트 트리 상에서 전역적으로 상태를 공유하고자 할때 사용하는 함수
// - useReducer : 상태 로직을 관리하는 함수 => 상태 업데이트 로직을 컴포넌트 외부로 옮길 수 있게하여 코드 관리를 더 수월하게 해줌
...
export default function HookComponent1() {
    ...
    // - 개발자 모드에서는 mount - unmount가 한번 실행되면서 테스트 후 다시 정상 실행 (마운트 -> 언마운트-> 마운트)

    // 컴포넌트 마운트시에만 실행
    // 홈페이지 처음에 들어갔을때 하는 작업
    useEffect(() => {
        // console.log("useEffect");
    }, []);

    // 스코프할 상태 배열을 지정하지 않으면 모든 상태가 변경될 때마다 실행 됨
    useEffect(() => {
        // 스코프할 상태 배열을 지정하지 않은 effect에서 상태를 변경하면 무한 실행됨(페이지 실행 무한 반복)
    ...
    // 스코프할 상태 배열에 두 개 이상의 상태를 지정할 수 있음 (배열에 포함된 상태 중 하나라도 변경되면 effect가 실행됨)
    useEffect(() => {
    ...
    const SubComponent = () => {
        ...
        // 전달한 콜백 함수에 retrun으로 컴포넌트가 unmount시 실행할 함수를 반환할 수 있음
